...
def get_td_error(states, next_states, actions, rewards, discount, terminals, network, current_q, tau):
    ...
    a = q_next_mat * probs_mat
    v_next_vec = np.sum (a,axis =1) * (1 - terminals)
    ### END CODE HERE
...
class Agent(BaseAgent):
    ...
    def agent_step(self, reward, state):
        ...
        ### START CODE HERE (~1 Line)
        new_experience = self.replay_buffer.append(self.last_state, self.last_action, reward, 0, state)
        ### END CODE HERE
        ...
        if self.replay_buffer.size() > self.replay_buffer.minibatch_size:
            ...
            for _ in range(self.num_replay):
                ...
                ### START CODE HERE
                optimize_network_update = optimize_network(experiences, self.discount, self.optimizer, self.network, current_q, self.tau)
                ### END CODE HERE
    ...
    def agent_end(self, reward):
        ...
        ### START CODE HERE (~1 Line)
        new_experience =  self.replay_buffer.append(self.last_state, self.last_action, reward, 1, state)
        ### END CODE HERE
